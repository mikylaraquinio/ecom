...
<x-app-layout>
  ...
  <style>
    ...
    /* Chat layout */
    #embedChat { height: 100vh; max-height: 100%; overflow: hidden; background: #fff; }
    #embedChat .hover-bg:hover { background:#f8f9fa; }
    #embedChat .side { width:260px; max-width:42%; }
    #embedChat .main { flex:1; background:#f5f5f5; min-width:0; }
    #embedChat .head { height:44px; display:flex; align-items:center; padding:0 .75rem; background:#fff; border-bottom:1px solid #eee; }
    #embedChat .msgs { flex:1; overflow:auto; padding:1rem; }
    #embedChat .bubble{ max-width:70%; padding:.5rem .75rem; border-radius:12px; }
    #embedChat .bubble.me{ background:#198754; color:#fff; }
    #embedChat .bubble.them{ background:#6c757d; color:#fff; }
    #embedChat .inputbar{ padding:.5rem; border-top:1px solid #eee; background:#fff; }
    @media (max-width:575.98px){ #embedChat .side{ width:50%; } }

    /* Optional: list-only mode (if you use ?list=1) */
    @if($listOnly)
      #embedChat .main { display: none !important; }
      #embedChat .side { width:100% !important; max-width:none !important; }
    @endif
  </style>
  ...
  <div id="embedChat" class="container-fluid">
    <div class="d-flex h-100">
        ...
        <div class="p-2 d-flex align-items-center gap-2 border-bottom">
          ...
          <div class="dropdown">
            <button class="btn btn-sm btn-outline-secondary dropdown-toggle"
                    ...
                    data-toggle="dropdown">     {{-- BS4 fallback --}}
              All
            </button>
      ...
      <main class="main d-flex flex-column">
        @if($receiver)
          <div class="head">
            <strong class="text-success">{{ $receiver->name }}</strong>
          </div>
