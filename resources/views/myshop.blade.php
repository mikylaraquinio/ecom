    <x-app-layout>
        <div class="container py-4">
            ...
            <div class="row g-3 mb-3">
                <div class="col-lg-6">
                    <div class="bg-white border rounded shadow-sm p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <a class="text-decoration-none small fw-semibold" href="#analytics" data-target="analytics">View Sales History ></a>
                        </div>

                        <div class="d-flex flex-wrap gap-2">
                            <a href="{{ url()->current() }}?status=pending#order-status"
                            ...
                            <a href="{{ url()->current() }}?status=canceled#order-status"
                            class="text-decoration-none">
                                <div class="border rounded p-2 text-center" style="width:110px;">
                                    <div class="fw-bold">0</div>
                            ...
                            <a href="{{ url()->current() }}?status=denied#order-status"
                            class="text-decoration-none">
                                <div class="border rounded p-2 text-center" style="width:110px;">
                                    <div class="fw-bold">0</div>
                            ...
                            <a href="{{ url()->current() }}?status=completed#order-status"
                            class="text-decoration-none">
                                <div class="border rounded p-2 text-center" style="width:110px;">
                                    <div class="fw-bold">0</div>
                ...
                <div class="col-lg-3 d-flex">
                    <div class="bg-white border rounded shadow-sm p-3 flex-fill">
                        ...
                        <a href="#order-status" data-target="order-status" class="nav-link text-decoration-none flex-shrink-0">
                            <div class="action-mini border rounded text-center d-flex flex-column align-items-center justify-content-center py-2 px-2" style="width:110px;">
                ...
                .action-mini i { font-size: 1rem; line-height: 1; }
                .action-mini .small { font-size: .8rem; }
                .actions-line { -webkit-overflow-scrolling: touch; } /* smooth scroll on mobile */
                </style>
            </div>
                                ...
                                <div class="d-flex justify-content-end">
                                    <form method="GET" action="{{ url()->current() }}#order-status" class="d-flex align-items-center gap-2">
                                        ...
                                        <select name="status" class="form-select form-select-sm" style="max-width: 220px;" onchange="this.form.submit()">
                                            ...
                                            <option value="shipped"          {{ request('status')==='shipped' ? 'selected' : '' }}>Shipped / Ready for Pickup</option>
                                                ...
                                                <td class="text-center">
                                                    @if($order->status == 'pending')
                                                        <div class="d-flex justify-content-center gap-2">
                                                            <form action="{{ route('seller.updateOrderStatus', $order->id) }}" method="POST" onsubmit="return confirm('Accept this order?')">
                                                                @csrf @method('PATCH')
                                                                <input type="hidden" name="status" value="accepted">
                                                                <button type="submit" class="btn btn-success btn-sm">Accept</button>
                                                            </form>
                                                            <form action="{{ route('seller.updateOrderStatus', $order->id) }}" method="POST" onsubmit="return confirm('Deny this order?')">
                                                                @csrf @method('PATCH')
        ...
        <tbody>
          @foreach($products as $product)
              ...
              <td>
                <div class="d-flex align-items-start gap-2">
                  ...
                  <div>
                    ...
                    <div class="text-muted xsmall">Item ID: <span class="text-body-secondary">{{ $product->id }}</span></div>
                  </div>
                </div>
              </td>
              ...
              <td class="text-start">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                  ...
                  <a href="#" class="link-primary small text-decoration-none"
                     data-bs-toggle="modal" data-bs-target="#editProductModal{{ $product->id }}">Edit</a>


                  <div class="dropstart">
                    <a href="#" class="link-secondary small text-decoration-none" data-bs-toggle="dropdown">More</a>
                    <ul class="dropdown-menu shadow-sm">
                      ...
                      <li>
                        <a class="dropdown-item text-danger" href="#"
                           data-bs-toggle="modal" data-bs-target="#confirmDelete{{ $product->id }}">
            ...
            {{-- View Product Modal (aesthetic, mirrors Add/Edit fields) --}}
                            @php
                              $viewSlides = [];
                              if (!empty($product->image)) $viewSlides[] = asset('storage/' . $product->image);
                              if (isset($product->images) && $product->images->count()) {
                                foreach ($product->images as $img) { $viewSlides[] = asset('storage/' . $img->path); }
                              }
                            @endphp
                                    ...
                                    <div class="row g-3">
                                      ...
                                      <div class="col-lg-4">
                                        <div class="border rounded-3 p-3 h-100">
                                          ...
                                          <div id="viewCarousel{{ $product->id }}" class="carousel slide mb-2" data-bs-touch="true">
                                            <div class="carousel-inner ratio ratio-1x1 border rounded bg-light overflow-hidden">
                                              @if(count($viewSlides))
                                                @foreach($viewSlides as $i => $src)
                                                  <div class="carousel-item @if($i===0) active @endif">
                                                    <img src="{{ $src }}" class="d-block w-100 h-100" style="object-fit:cover;" alt="Image {{ $i+1 }}">
                                                  </div>
                                                @endforeach
                                              @else
                                            ...
                                            <button class="carousel-control-prev" type="button" data-bs-target="#viewCarousel{{ $product->id }}" data-bs-slide="prev">
                                              <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                              <span class="visually-hidden">Previous</span>
                                            </button>
                                            <button class="carousel-control-next" type="button" data-bs-target="#viewCarousel{{ $product->id }}" data-bs-slide="next">
                                        ...
                                        <div class="col-lg-8">
                                          <div class="px-lg-1">
                                            ...
                                            <div class="mb-3">
                                              ...
                                              <div class="text-body" style="white-space:pre-wrap">{{ $product->description ?? '—' }}</div>
                                            </div>
                                  ...
                                  <div class="modal-footer border-0 pt-0">
                                    ...
                                    <button type="button" class="btn btn-primary"
                                      ...
                                      <i class="fas fa-edit me-1"></i> Edit
                                        ...
                                        @php
                                          $existingSlides = [];
                                          if (!empty($product->image)) $existingSlides[] = asset('storage/' . $product->image);
                                          if (isset($product->images) && $product->images->count()) {
                                            foreach ($product->images as $img) { $existingSlides[] = asset('storage/' . $img->path); }
                                          }
                                        @endphp
                                        <div class="modal fade edit-product-modal"
                                            id="editProductModal{{ $product->id }}"
                                            data-pid="{{ $product->id }}"
                                            tabindex="-1" aria-labelledby="editProductLabel{{ $product->id }}" aria-hidden="true">
                                          <div class="modal-dialog modal-xl modal-dialog-scrollable">
                                            <div class="modal-content">
                                              ...
                                              <form class="needs-validation" novalidate method="POST"
                                                  ...
                                                  <div class="row g-3">
                                                    ...
                                                    <div class="col-lg-4">
                                                      <div class="border rounded-3 p-3 h-100">
                                                        ...
                                                        <div id="editPreviewCarousel{{ $product->id }}" class="carousel slide mb-2" data-bs-touch="true">
                                                          <div class="carousel-inner ratio ratio-1x1 border rounded bg-light overflow-hidden"
                                                            ...
                                                            @if(count($existingSlides))
                                                              ...
                                                              <div class="carousel-item @if($idx===0) active @endif">
                                                                <img src="{{ $src }}" class="d-block w-100 h-100" style="object-fit:cover;" alt="Image {{ $idx+1 }}">
                                                              </div>
                                                              @endforeach
                                                            @else
                                                              <div class="carousel-item active d-flex align-items-center justify-content-center">
                                                          ...
                                                          <button class="carousel-control-prev" type="button" data-bs-target="#editPreviewCarousel{{ $product->id }}" data-bs-slide="prev">
                                                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                                            <span class="visually-hidden">Previous</span>
                                                          </button>
                                                          <button class="carousel-control-next" type="button" data-bs-target="#editPreviewCarousel{{ $product->id }}" data-bs-slide="next">
                                                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                                            <span class="visually-hidden">Next</span>
                                                          </button>
                                                        </div>
                                                        ...
                                                        <div>
                                                          ...
                                                          <div class="small">
                                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                                              ...
                                                              <span id="pv_name_{{ $product->id }}" class="fw-semibold">{{ $product->name }}</span>
                                                            </div>
                                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                                              <span class="text-muted">Price:</span>
                                                              <span id="pv_price_{{ $product->id }}" class="fw-semibold">₱{{ number_format($product->price, 2) }}</span>
                                                            </div>
                                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                                              <span class="text-muted">Unit:</span>
                                                              <span id="pv_unit_{{ $product->id }}" class="fw-semibold">{{ $product->unit ?? '—' }}</span>
                                                            </div>
                                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                                              <span class="text-muted">Stock:</span>
                                                              <span id="pv_stock_{{ $product->id }}" class="fw-semibold">{{ $product->stock }}</span>
                                                            </div>
                                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                                              <span class="text-muted">Min order qty:</span>
                                                              <span id="pv_moq_{{ $product->id }}" class="fw-semibold">{{ $product->min_order_qty ?? '—' }}</span>
                                                            </div>
                                                            <div class="d-flex justify-content-between py-1">
                                                              <span class="text-muted">Weight / unit:</span>
                                                              <span id="pv_weight_{{ $product->id }}" class="fw-semibold">
                                                                @if(isset($product->weight)) {{ number_format($product->weight, 2) }} kg @else — @endif
                                                              </span>
                                                            </div>
                                                          </div>
                                                        </div>
                                                      ...
                                                      <div class="px-lg-1">
                                                        ...
                                                        <div id="shpGrid{{ $product->id }}" class="shp-grid">
                                                          ...
                                                          @foreach($existingList as $idx => $img)
                                                            <div class="shp-item" draggable="true"
                                                                data-type="existing"
                                                                @if($img['id']) data-existing-id="{{ $img['id'] }}" @endif>
                                                          ...
                                                          {{-- Add tile --}}
                                                          <label class="shp-add">
                                                            <input id="shpPicker{{ $product->id }}" type="file" accept="image/*" multiple hidden>
                                                        ...
                                                        <input id="shpMainFile{{ $product->id }}" name="image" type="file" class="d-none">           {{-- main (first new file) --}}
                                                        <input id="shpGalleryFiles{{ $product->id }}" name="gallery[]" type="file" class="d-none" multiple> {{-- remaining new files --}}
                                                        {{-- Track existing re-order (IDs or "main") and removals --}}
                                                        <div id="shpExistingWrap{{ $product->id }}"></div>  {{-- will be filled with existing_order[] and remove_existing[] --}}
                                                        <div class="form-text">Up to 9 images. Drag to reorder. First image is the Cover.</div>


                                                        <label class="form-label fw-semibold">Product Name <span class="text-danger">*</span></label>
                                                        <input id="fld_name_{{ $product->id }}" type="text" class="form-control" name="name" value="{{ $product->name }}" maxlength="120" required>
                                                        <div class="invalid-feedback">Product name is required.</div>

                                                        <label class="form-label fw-semibold mt-3">Description <span class="text-danger">*</span></label>
                                                        <textarea id="fld_desc_{{ $product->id }}" class="form-control" name="description" rows="4" maxlength="800" required>{{ $product->description }}</textarea>
                                                        ...
                                                        <div class="row g-3 mt-1">
                                                          <div class="col-md-4">
                                                            ...
                                                            <div class="input-group">
                                                              ...
                                                              <input id="fld_price_{{ $product->id }}" type="number" class="form-control" name="price" step="0.01" min="0.01" value="{{ $product->price }}" required>
                                                            </div>
                                                            <div class="invalid-feedback">Enter a valid price.</div>
                                                          </div>
                                                          <div class="col-md-4">
                                                            <label class="form-label fw-semibold">Unit <span class="text-danger">*</span></label>
                                                            <select id="fld_unit_{{ $product->id }}" class="form-select" name="unit" required>
                                                              ...
                                                              <option value="sack"  {{ ($product->unit ?? '')==='sack' ? 'selected' : '' }}>Sack</option>
                                                            </select>
                                                          ...
                                                          <div class="col-md-4">
                                                            ...
                                                            <input id="fld_stock_{{ $product->id }}" type="number" class="form-control" name="stock" min="0" step="1" value="{{ $product->stock }}" required>
                                                        ...
                                                        <div class="row g-3">
                                                          <div class="col-md-6 col-lg-5">
                                                            ...
                                                            <input id="fld_moq_{{ $product->id }}" type="number" class="form-control" name="min_order_qty" min="1" step="1" value="{{ $product->min_order_qty }}">
                                                          </div>
                                                          <div class="col-md-6 col-lg-5">
                                                            <label class="form-label fw-semibold">Weight per unit <span class="text-danger">*</span></label>
                                                            <div class="input-group">
                                                              <input id="fld_weight_{{ $product->id }}" type="number" class="form-control" name="weight" step="0.01" min="0.01" value="{{ $product->weight }}" required>
                                                ...
                                                <div id="shpExistingWrap{{ $product->id }}"></div>
<input type="file" id="shpMainFile{{ $product->id }}" name="image" class="d-none">
<input type="file" id="shpGalleryFiles{{ $product->id }}" name="gallery[]" class="d-none" multiple>
                                              </form>
                                            </div>
                                          </div>
                                        </div>
                                  ...
                                  <div class="row g-3">
                                    ...
                                    <div class="col-lg-4">
                                      <div class="border rounded-3 p-3 h-100">
                                        ...
                                        <div id="previewCarousel" class="carousel slide mb-2" data-bs-touch="true">
                                          <div class="carousel-inner ratio ratio-1x1 border rounded bg-light overflow-hidden" id="carouselInner">
                                            <div class="carousel-item active d-flex align-items-center justify-content-center">
                                              <span class="text-muted">No image yet</span>
                                            </div>
                                          </div>
                                        ...
                                        <div>
                                          ...
                                          <div class="small">
                                            ...
                                            <div class="d-flex justify-content-between py-1 border-bottom">
                                              <span class="text-muted">Min order qty:</span><span id="pv_moq" class="fw-semibold">—</span>
                                            </div>
                                            <div class="d-flex justify-content-between py-1">
                                              <span class="text-muted">Weight / unit:</span><span id="pv_weight" class="fw-semibold">—</span>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    ...
                                    <div class="col-lg-8">
                                      <div class="px-lg-1">
                                        ...
                                        <div class="form-text">Up to 9 images. Drag to reorder. First image is the Cover.</div>
                                        <div class="invalid-feedback d-block" id="shpAddValidation" style="display:none;">Please select at least one image.</div>
                                        ...
                                        <label class="form-label fw-semibold mt-3">Description <span class="text-danger">*</span></label>
                                        <textarea id="fld_desc" class="form-control" name="description" rows="4" maxlength="800" required></textarea>
                                        <div class="invalid-feedback">Please add a short description.</div>
                                        ...
                                        <div class="row g-3 mt-1">
                                          <div class="col-md-4">
                                            ...
                                            <div class="input-group">
                                              <span class="input-group-text">₱</span>
                                              <input id="fld_price" type="number" class="form-control" name="price" step="0.01" min="0.01" required>
                                            </div>
                                            <div class="invalid-feedback">Enter a valid price.</div>
                                          </div>
                                          <div class="col-md-4">
                                          ...
                                          <div class="col-md-4">
                                            <label class="form-label fw-semibold">Stock <span class="text-danger">*</span></label>
                                            <input id="fld_stock" type="number" class="form-control" name="stock" min="0" step="1" required>
                                            <div class="invalid-feedback">Provide available stock.</div>
                                          </div>
                                        </div>
                                        ...
                                        <div class="row g-3">
                                          <div class="col-md-6 col-lg-5">
                                            <label class="form-label fw-semibold">Min order quantity</label>
                                            <input id="fld_moq" type="number" class="form-control" name="min_order_qty" min="1" step="1" placeholder="Optional">
                                          </div>
                                          <div class="col-md-6 col-lg-5">
                                            <label class="form-label fw-semibold">Weight per unit <span class="text-danger">*</span></label>
                                            <div class="input-group">
                                              <input id="fld_weight" type="number" class="form-control" name="weight" step="0.01" min="0.01" required>
                                              <span class="input-group-text">kg</span>
                                            </div>
                                            <small class="text-muted">Example: 1 mango ≈ 0.25 kg</small>
                                            <div class="invalid-feedback">Enter weight per unit.</div>
                        ...
                        <style>
                          ...
                          #previewCarousel .carousel-inner img { object-fit: cover; width: 100%; height: 100%; }
                          .modal-dialog-scrollable .modal-body { max-height: 68vh; }
                        </style>
        ...
        .row.g-3.mb-3 { align-items: stretch; }
        .row.g-3.mb-3 > [class*="col-"] { display:flex; }
        .row.g-3.mb-3 > [class*="col-"] > .bg-white { flex:1 1 auto; }

        /* centered, balanced dropdown button */
        .action-menu-btn{
            display:flex;
            align-items:center;
            justify-content:center;   /* center icon + label + caret */
            gap:.625rem;
            min-width: 240px;         /* nice readable width */
            height: 44px;
            padding:.5rem .75rem;
  ...
  @media (min-width: 576px) { .shp-grid { grid-template-columns: repeat(4, 1fr); } }
  @media (min-width: 768px) { .shp-grid { grid-template-columns: repeat(5, 1fr); } }
  ...
  .shp-item:hover, .shp-add:hover { box-shadow: 0 .35rem .8rem rgba(0,0,0,.06); transform: translateY(-1px); }
  .shp-item.dragging { opacity: .6; border-color: #0d6efd; }
  .shp-item img { width: 100%; height: 100%; object-fit: cover; display: block; }
  ...
  .shp-cover {
    ...
    background: #0d6efd; color: #fff; font-size: .7rem;
    border-radius: .35rem; padding: .15rem .4rem;
  }

  .shp-add { display: grid; place-items: center; cursor: pointer; }
  .shp-add-inner { text-align: center; color: #6c757d; }
  .shp-plus { font-size: 1.75rem; line-height: 1; }
</style>
...
<script>
document.addEventListener('DOMContentLoaded', function () {
  // Initialize once when Add Product modal opens
  ...
  addModalEl.addEventListener('shown.bs.modal', function () {
    if (addModalEl.dataset.shopeeInit) return;
    addModalEl.dataset.shopeeInit = '1';
    ...
    const mainH    = addModalEl.querySelector('#shpAddMainFile');
    const galH     = addModalEl.querySelector('#shpAddGalleryFiles');
    const errEl    = addModalEl.querySelector('#shpAddValidation');

    // Left preview carousel (already present in your modal)
    const inner    = addModalEl.querySelector('#carouselInner');

    // Local state = array of { file, src }
    let filesState = [];
    ...
    function rebuildCarousel() {
      ...
      filesState.forEach((obj, idx) => {
        const slide = document.createElement('div');
        slide.className = 'carousel-item' + (idx===0 ? ' active' : '');
        slide.innerHTML = `<img src="${obj.src}" class="d-block w-100 h-100" style="object-fit:cover;" alt="Preview ${idx+1}">`;
        inner.appendChild(slide);
      });
    }
    ...
    function makeTileForFile(file) {
      ...
      tile.innerHTML = `
        ...
        <button type="button" class="shp-remove" aria-label="Remove">&times;</button>`;
      // Wire remove
      tile.querySelector('.shp-remove').addEventListener('click', function() {
        const idx = [...grid.querySelectorAll('.shp-item')].indexOf(tile);
        if (idx > -1) { filesState.splice(idx,1); tile.remove(); refreshCountAndCover(); rebuildCarousel(); mapToHidden(); }
      });
      // Drag events
      tile.addEventListener('dragstart', e => { tile.classList.add('dragging'); e.dataTransfer.effectAllowed='move'; });
      tile.addEventListener('dragend',   () => { tile.classList.remove('dragging'); updateStateFromDOM(); });
      return {tile, url, file};
    }

    function updateStateFromDOM() {
      // Rebuild filesState order by DOM order using blob URLs
      const mapBySrc = new Map(filesState.map(o => [o.src, o]));
      const newState = [];
      grid.querySelectorAll('.shp-item img').forEach(img => {
        const obj = mapBySrc.get(img.src);
    ...
    // Grid drag-over to sort position
    ...
    grid.addEventListener('drop', function() { updateStateFromDOM(); });
    ...
    picker.addEventListener('change', function() {
      ...
      toAdd.forEach(file => {
        const {tile, url} = makeTileForFile(file);
        // Keep an object with {file, src:url}
        filesState.push({file, src:url});
        // Insert before Add tile
        const addTile = grid.querySelector('.shp-add');
        grid.insertBefore(tile, addTile);
      });
      // Reset picker so same files can be re-picked later
      picker.value = '';
      refreshCountAndCover();
      rebuildCarousel();
      mapToHidden();
    ...
    // Map state -> hidden file inputs (main + gallery)
    function mapToHidden() {
      const mainDT = new DataTransfer();
      const galDT  = new DataTransfer();
      if (filesState.length > 0) {
        mainDT.items.add(filesState[0].file);
        filesState.slice(1).forEach(o => galDT.items.add(o.file));
      }
      mainH.files = mainDT.files;
      galH.files  = galDT.files;
    }

    // On submit, ensure at least one image
    ...
    form.addEventListener('submit', function(e) {
      ...
      // Hidden inputs already prepared by mapToHidden()
    });
