<x-app-layout>
  <div class="container mt-5">
      ...
      <div class="row">
        ...
        <div class="col-12">
          ...
          @foreach($groups as $sellerId => $items)
            ...
            <div class="card mb-3 shop-block" data-seller-id="{{ $sellerId }}"
              data-shipping="{{ $shippingBySeller[$sellerId]['shippingFee'] ?? 0 }}">
              ...
              <div class="card-footer bg-white small text-muted d-flex align-items-center shop-shipping-footer"
                id="shop-shipping-{{ $sellerId }}">
                <i class="fas fa-truck me-2"></i>
                Shipping: ₱<span
                  class="shop-shipping-fee">{{ number_format($shippingBySeller[$sellerId]['shippingFee'] ?? 0, 2) }}</span>
                <span class="ms-2">(Weight: <span
                    class="shop-shipping-weight">{{ $shippingBySeller[$sellerId]['weight'] ?? 0 }}</span> kg)</span>
              </div>
          @endforeach
          </div>
        </div>
        ...
        <div class="cart-bottom-bar shadow-lg">
          <div class="container d-flex align-items-center flex-wrap gap-3 py-2">
            ...
            <button type="button" class="btn btn-outline-danger btn-sm" id="delete-selected-bottom"
              disabled>Remove</button>

            <div class="ms-auto text-end">
              <div class="small text-muted shipping-note">
                Shipping: ₱{{ number_format($totalShipping, 2) }}
              </div>
              <div class="h5 m-0">
                Grand Total: ₱<span id="total-price">{{ number_format($grandTotal, 2) }}</span>
              </div>
            </div>
    ...
    <style>
      ...
      .shop-block .list-group-item {
        border-left: 0;
        border-right: 0;
      }

      .shop-block .card-header {
